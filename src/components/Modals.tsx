...
export function ConfigModal({
  script, paramValues, updateParam, onClose,
}: {
  script: ScriptItem;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
        <div className="mb-3 flex items-start justify-between">
          ...
          <button onClick={onClose} className="rounded-md border px-2 py-1 text-xs text-gray-600 hover:bg-gray-50">ปิด</button>
        </div>

        <div className="space-y-4 text-sm">
          <div className="flex items-center justify-between text-xs text-gray-500">
            <span className="rounded-full bg-gray-100 px-2 py-0.5 font-medium capitalize">{script.target}</span>
            <span>อัปเดต {script.lastUpdated}</span>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
        ...
        <div className="space-y-3 text-sm">
          ...
          {Array.isArray(script.examples) && script.examples.length > 0 && (
            <div>
              ...
              <ul className="list-disc space-y-1 pl-5 text-gray-700">
                {script.examples.map((ex, i) => (<li key={i} className="break-words">{ex}</li>))}
              </ul>
            </div>
          )}
          ...
          {script.docs_url && (
            <a href={script.docs_url} target="_blank" rel="noreferrer" className="inline-block rounded-lg border border-indigo-200 px-3 py-2 text-xs font-medium text-indigo-700 hover:bg-indigo-50">
              เปิดเอกสารประกอบ
            </a>
          )}
        </div>
